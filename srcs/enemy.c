...
/*   Updated: 2023/08/05 10:54:42 by micheng          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_enemy(t_vars *vars)
{
	...
	while (current != NULL)
	{
		if (vars->play_dead == 1 || (vars->enemy_trapped == 1
				&& vars->enemy_trapped_count >= 0))
		{
			vars->enemy_trapped_count--;
			if (vars->enemy_trapped_count == 0)
